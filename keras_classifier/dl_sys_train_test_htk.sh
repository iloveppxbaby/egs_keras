...
genuine_mdl_name=dl_htk_genuine
mdl_name=${genuine_mdl_name}
mdl_tail=${fea_tail}
train_ndx=${list_dir}/dl_htk_train.ndx
>${train_ndx}
...
# python ./eng_bins/dl_train_htk.py ${train_ndx} ${fea_head} ${fea_tail} ${mdl_head} ${mdl_tail} ${classifier}
...
seg_list=${list_dir}/dl_htk_test.seg
cat ${raw_list} | cut -d" " -f1   > ${seg_list}
python ./eng_bins/dl_test_htk.py ${classifier} ${mdl_head} ${mdl_tail} ${seg_list} ${fea_head} ${fea_tail} ${score}
...
key=${list_dir}/dl_htk_asvspoof_dev.key
